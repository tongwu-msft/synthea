...
class MissingProcessor:
    ...
    def fit(self, data):
        ...
        if loc.any():
            ...
            use_data = data
        res = self.model.fit_transform(use_data.reshape(-1,1))
        if isinstance(self.model, MinMaxScaler):
            self.mean = np.mean(use_data)
            self.std = np.std(use_data)
    ...
    def transform(self, data, times=None):
        if self.missing:
            ...
            if self.dtype == object:
                loc = np.array([x!=x for x in data])
                if isinstance(self.model, LabelBinarizer):
                    filldata = np.array([str(self.non) if x!=x else str(x) for x in data])
                else:
    ...
    def fit(self, data):
        ...
        for i, (types, col) in enumerate(zip(self.types,data.columns)):
            ...
            if types in ['continuous', 'int']:
                print(model.model.data_max_, model.model.data_min_, model.mean, model.std)
