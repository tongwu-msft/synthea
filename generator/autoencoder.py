...
class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, embed_dim, layers, dropout):
        ...
        #self.factor = nn.Parameter(torch.rand((layers, 2 * layers)))
    ...
    def forward(self, statics, dynamics, priv, nex, mask, times, seq_len):
        ...
        """ 
        lasth = self.factor.view(self.layers, 2 * self.layers, 1, 1) * h.view(1, 2*self.layers, bs, self.hidden_dim)
        lasth = torch.sum(lasth, dim=1)
        """
...
class Pattern(nn.Module):
    def __init__(self, miss_dim, hidden_dim):
        super(Pattern, self).__init__()
        self.fc = nn.Linear(hidden_dim, hidden_dim)
        self.pat = nn.Parameter(torch.rand((miss_dim, hidden_dim)) - 0.5)
        #self.fc1 = nn.Linear(hidden_dim, hidden_dim)
        #self.norm = nn.LayerNorm(hidden_dim)
        
    def forward(self, h):
        q = self.fc(h)
        k = self.pat.transpose(0,1).contiguous().unsqueeze(0)
        v = self.pat.unsqueeze(0)
        score = torch.softmax(torch.matmul(q, k), dim=-1)
        r = torch.matmul(score, v)
        #r = self.norm(self.fc1(r))
...
class Decoder(nn.Module):
    def __init__(self, processors, hidden_dim, layers, dropout):
        ...
        #self.pat = Pattern(self.miss_dim, hidden_dim)
        
        self.miss_fc = nn.Linear(hidden_dim, self.miss_dim)
        self.time_fc = nn.Linear(hidden_dim, 1)
        #self.factor = nn.Linear(self.miss_dim, hidden_dim)
